			...
			test = false;
				...
				if (type == 0) { // land on a car that is available for purchase
					...
					if (decision == 'Y' or decision == 'y') {
						...
						if (currentSpace == 12 or currentSpace == 28) {
					...
					else if (players[i].retrieveWealth() < rental and !mortgage) {
					...
					if (players[i].returnSpace() == 4) {
						...
						if (decision == 'M' or decision == 'm') { //choose to pay $40,000
						...
						else if (decision == 'A' or decision == 'a') { //choose to pay 10% of all assets
							int assets = 0;
