...
void Game_Board::updateUpdate(Player* player, Bank* bank) {
	...
	if (counter == 0) {
		...
		do {
			...
			else if (decision < 0 or decision > counter) {
				cin.clear();
				cin.ignore(10000, '\n');
...
int Game_Board::retrieveNumberofUpgrades(Player player) {
	int counter = 0;
	for (int i = 0; i < 40; i++) {
		if (i != 5 and i != 15 and i != 25 and i != 35 and i != 12 and i != 28) {
			int temp = player.retrieveProperty(i);
			if (temp == 1) {
...
void Game_Board::outputRow(int start, int end){ //output entire row
	...
	for (int i = 0; i < 11; i++ ){
		...
		int counter = counterIncrement(start, end, i);
		if (allSpaces[counter].returnPurchased() == '1' or allSpaces[counter].returnPurchased() == '2'
		 or allSpaces[counter].returnPurchased() =='3' or allSpaces[counter].returnPurchased() =='4') {
...
void Game_Board::outputColumn(int left, int right, bool bottom) {
	...
	if (allSpaces[left].returnPurchased() == '1' or allSpaces[left].returnPurchased() =='2'
	 or allSpaces[left].returnPurchased() =='3' or allSpaces[left].returnPurchased() =='4') {
	...
	cout << "                 |";
	if (allSpaces[right].returnPurchased() == '1' or allSpaces[right].returnPurchased() =='2' or 
		allSpaces[right].returnPurchased() =='3' or allSpaces[right].returnPurchased() =='4') {
